...
void list_map1 (void (*f) (element_t*, element_t), struct list* out_list, struct list* in_list) {
  if(in_list->len > out_list->size){
    expand_list(out_list);
  }
...
void test_map1(void (*f) (intptr_t*, intptr_t), struct list* out_list, struct list* in_list) {
  for(int i =0; i<in_list->len;i++){
    f((intptr_t*)&out_list->data[i], (intptr_t) in_list->data[i]);
  }
}
*/
/**
...
void list_map2 (void (*f) (element_t*, element_t, element_t), struct list* out_list, struct list* in_list0, struct list* in_list1) {
  // TODO
  int length = (in_list0->len>in_list1->len)? in_list1->len : in_list0->len;
  if(length > out_list->size){
    expand_list(out_list);
  }
...
void test_foreach(void (*f) (intptr_t), struct list* list) {
  for (int i = 0; i < list->len; i++){
    f ((intptr_t)list->data [i]);
  }
}



void inc(element_t* rpv, element_t a) {
